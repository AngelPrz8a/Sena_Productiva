...
<h6>Nivel: {{$programa->Nivel}}</h6>
<h6>Estado: {{$programa->Estado}}</h6>
        ...
        @foreach ($programa->fichas()->get() as $fichas)
            ...
            <div class="col-sm-2">
                <div class="card" style="width: 10rem;">
                    ...
                    <div class="card-body">
                        <h5 class="card-title">
                            <a style="text-decoration: none;" href="{{  url('fichas/'.$fichas->IdFicha)  }}">
                                {{$fichas->NumeroFicha}}
                            </a>
                        </h5>

                        <p class="card-text">
                            @if ($fichas->InicioProductiva < Carbon\Carbon::now() ) {{--Si la fecha de inicio Productiva es menor a la actual, es etapa lectiva, sino es etapa productiva--}}
                            {{ 'lectiva'}}
                            @else
                            {{'productiva'}}
                            @endif
                        </p>
                            <a style="text-decoration: none;" href="{{  url('fichas/'.$fichas->IdFicha)  }}" class="card-link">
                            ...
                            <a style="text-decoration: none;"  href="{{  url('fichas/'.$fichas->IdFicha.'/edit')  }}" class="card-link">
